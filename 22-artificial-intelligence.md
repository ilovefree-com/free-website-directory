# Artificial Intelligence

🌐 **View on iLoveFree.com**: https://www.ilovefree.com/free-website-navigation/artificial-intelligence/

[⬆️ Back to README](./README.md)

## Artificial Intelligence | I Love Free

> Curated free AI tools and resources: chatbots, writing assistants, video and image generators, audio tools, and AI indexes from the FMHY community.

### AI Chatbots

#### Official Model Sites

-   [DeepSeek](https://chat.deepseek.com/) - V4-Pro (Expert) / V4-Flash (Instant) / Sign-Up / Unlimited
-   [Gemini](https://gemini.google.com/) - Gemini 3.1 Pro Preview (5 daily) / Gemini 3 Flash (unlimited) / Add Features / Limits
-   [Google AI Studio](https://aistudio.google.com/app/prompts/new_chat) - Gemini 3.1 Pro / Google Sign-Up / Add Features
-   [Kimi](https://www.kimi.com/) - Kimi K2.6 (Thinking) / Slides / Google Login or Phone # Required
-   [Qwen](https://chat.qwen.ai/) - Qwen3.6-Plus / Qwen3.5-Plus / Sign-Up
-   [⁠Z.ai](https://chat.z.ai/) - GLM-5.1 / GLM-5-Turbo / Slides
-   [⁠Ai2 Playground](https://playground.allenai.org/) - Olmo 3.1 32B (Think) / No Sign-Up / Unlimited
-   [Apertus](https://publicai.co/chat) - Apertus 70B
-   [ArceeAI](https://chat.arcee.ai/chat)
-   [ChatGPT](https://chatgpt.com/) - GPT-5.5 Instant / r/OpenAI / r/ChatGPT
-   [Claude](https://claude.ai/) - Claude Sonnet 4.6 / Require Sign-Up + Phone # / Resources / Learning, 2 / Usage Tracker
-   [⁠Cohere](https://dashboard.cohere.com/playground/chat) - Command A Reasoning / 1000 Monthly / Sign-Up
-   [⁠Ernie](https://ernie.baidu.com/) - Ernie 5.1 Preview / ERNIE 5.0
-   [Grok](https://grok.com/) - Grok 4.2 Fast-Non-Reasoning / Grok 4.2 Fast-Reasoning / Traffic Based / Rate Display, 2
-   [⁠Hy3-preview](https://huggingface.co/spaces/tencent/Hy3-preview) - ⁠Hy3-preview / Unlimited
-   [⁠Inception Chat](https://chat.inceptionlabs.ai/) - Mercury 2 / Unlimited
-   [⁠K2Think](https://www.k2think.ai/) - LLM360 / MBZUAI (not Kimi) / Sign-Up
-   [⁠LongCat](https://longcat.chat/) - LongCat Flash Reasoning / Sign-Up / Unlimited
-   [Meta AI](https://www.meta.ai/) - Muse Spark / Meta Sign-Up / Unlimited
-   [Microsoft Copilot](https://copilot.microsoft.com/) - GPT-5.1 / Unlimited
-   [⁠MiMo Studio](https://aistudio.xiaomimimo.com/) - MiMo-V2.5-Pro / MiMo-V2.5 / Sign-Up
-   [MiniMax AI](https://agent.minimax.io/) - MiniMax-M2.7
-   [Mistral](https://chat.mistral.ai/) - Mistral Large 3
-   [Reka](https://www.reka.ai/) - Reka Flash 3.1 / Sign-Up
-   [Sarvam](https://dashboard.sarvam.ai/chat) - Sarvam-105b / Sarvam-30b / Unlimited / Sign-Up
-   [⁠Solar](https://console.upstage.ai/playground/chat) - Solar Pro 3 / No Sign-Up

#### Multiple Model Sites

-   [AI Assistant](https://aiassistantbot.pages.dev/) - Deepseek V3.2 / GLM-4.7 / Kimi K2.5 / Multiple Models / No Sign-Up
-   [Arena](https://arena.ai/text/direct) - Multiple Models / Sign-Up / Reset Limits w/ Temp Mail / OpenAI Bridge
-   [Cerebras Chat](https://chat.cerebras.ai/) - GLM-4.7 / Qwen3-235B / Sign-Up
-   [Dolphin Chat](https://chat.dphn.ai/) - Dolphin 24B / Unrestricted / No Sign Up / Unlimited
-   [⁠Element Flash](https://flash.kernal.bid/) - Multiple Models / Queue (if traffic is high) / Sign-Up
-   [Groq](https://console.groq.com/playground) - Kimi K2-0905 / GPT-OSS-120B / Sign-Up / Limits
-   [⁠Indic LLM Arena](https://arena.ai4bharat.org/#/chat) - Multiple Models / Unlimited
-   [⁠ISH](https://beta.ish.chat/) - Claude Opus 4.6 / Claude Sonnet 4.6 / Multiple Models / Sign-Up (GitHub)
-   [NVIDIA NIM](https://build.nvidia.com/models) - GLM-5.1 / Deepseek-v4-Pro / Kimi K2.6 / Multiple Models / No Sign-Up / Unlimited
-   [Pollinations Chat](https://pollinations.ai/play) - Grok 4.20 Non-Reasoning / GLM-5.1 / GPT-5.4 / Multiple Models / Limits and Models
-   [⁠Together.ai](https://chat.together.ai/) - GLM-5.1 / Kimi K2.6 / Deepseek-V4 / Multiple Models / Up To 110 Daily
-   [⁠Woozlit](https://woozlit.com/) - Opus 4.7 / Sonnet 4.6/ Gemini 3.1 Pro / DeepSeek-v4-Pro / Multiple Models / ≈2000 Monthly / Sign-Up

#### Specialized Chatbots

-   [⁠Ask Brave](https://search.brave.com/ask) - AI Search Engine
-   [Google AI Mode](https://google.com/aimode) - AI Search Engine / Pro (Google Signup) / Fast (No Signup)
-   [NotebookLM](https://notebooklm.google.com/) - Document Chatbot / Note-Taking / Slides / Anki Export
-   [Alphaxiv](https://www.alphaxiv.org/) - Research Chatbot
-   [⁠Anara](https://anara.com/) - Document Chatbot
-   [Arena](https://arena.ai/?mode=direct&chat-modality=search) - AI Search Engine / Sign-Up / Reset Limits w/ Temp Mail / OpenAI Bridge
-   [Awesome AI Web Search](https://github.com/felladrin/awesome-ai-web-search) - AI Search Engine Index
-   [DocsGPT](https://www.docsgpt.cloud/) - Document Chatbot / Note-Taking
-   [Elicit](https://elicit.com/) - Research Chatbot
-   [⁠Exa](https://exa.ai/search) - AI Search Engine
-   [Hyperspace](https://hyper.space/) - P2P AI Network
-   [⁠iAsk AI](https://iask.ai/) - Research Chatbot
-   [Learn About](https://learning.google.com/experiments/learn-about) - Google's Educational Search AIs / Add Features
-   [Onyx](https://www.onyx.app/) - Document Chatbot / Note-Taking
-   [⁠Paper2Gal](https://paper2gal.com/) - Turn Academic Papers into Visual Novels via AI
-   [Perplexity](https://www.perplexity.ai/) - AI Search Engine / Enhancements
-   [PrivateGPT](https://privategpt.dev/) - Document Chatbot / Note-Taking
-   [⁠Sci-Bot](https://sci-bot.ru/) - Sci-Hub Research Chatbot
-   [Scinito](https://ekb.scinito.ai/ai/chat) - Research Chatbot
-   [SciSpace](https://scispace.com/) - Research Chatbot
-   [Vane](https://github.com/ItzCrazyKns/Vane) - FOSS AI Search Engine / Self-Hosted

#### Local AI Frontends

-   [Jan](https://jan.ai/) - Desktop App
-   [LM Studio](https://lmstudio.ai/) - Desktop App / Self-Hosted Models Only
-   [oobabooga](https://github.com/oobabooga/textgen) - Web UI / Self-Hosted Models Only / Colab
-   [Open WebUI](https://openwebui.com/) - Web UI
-   [SillyTavern](https://docs.sillytavern.app/) - Desktop App / Presets / Themes
-   [AnythingLLM](https://anythingllm.com/) - Desktop App
-   [⁠ch.at](https://github.com/Deep-ai-inc/ch.at) - Language Model Bridge / Lightweight / Minimal
-   [Chatbox](https://chatboxai.app/) - Desktop App
-   [Cherry Studio](https://www.cherry-ai.com/) - Desktop App / Mobile
-   [GPT4All](https://www.nomic.ai/gpt4all) - Self-Hosted
-   [GPT4Free](https://github.com/xtekky/gpt4free) - Desktop App / Web
-   [LibreChat](https://librechat.ai/) - Web UI
-   [LobeHub](https://lobechat.com/chat) - Desktop App / Requires Sign-Up
-   [LoLLMs](https://lollms.com/) - Web UI
-   [Msty](https://msty.app/) - Desktop App
-   [Noi](https://noib.app/) - Desktop App
-   [tgpt](https://github.com/aandrew-me/tgpt) - Command-Line Interface

#### Self-Hosting Tools

-   [Aphrodite Engine](https://aphrodite.pygmalion.chat/) - Serve LLMs at Scale with Kobold & OpenAI APIs / Colab
-   [KoboldCpp](https://github.com/LostRuins/koboldcpp) - llama.cpp with API + GUI / ROCm / Colab
-   [llama.cpp](https://github.com/ggerganov/llama.cpp) - Self-Hosted Transformer-Based LLMs
-   [Awesome Local LLM](https://github.com/rafska/awesome-local-llm/) - Local LLM Platforms / Resources
-   [⁠Can I Run AI Locally](https://www.canirun.ai/) - Determine Which AI Models You Can Run Locally
-   [Leon](https://getleon.ai/) - Self-Hosted
-   [llamafile](https://github.com/Mozilla-Ocho/llamafile) - Run LLM with Single Files
-   [LLM-API-Key-Proxy](https://github.com/Mirrowel/LLM-API-Key-Proxy) - LLM API Key Proxy
-   [⁠LocalAI](https://localai.io/) - Self-Hosted Models
-   [Ollama](https://ollama.com/) - Self-Hosted
-   [Petals](https://petals.dev/) - Self-Hosted

#### Roleplaying Chatbots

-   [Agnai](https://agnai.chat/)
-   [Chub](https://chub.ai/) - Roleplaying Chatbots / Sign-Up / Some NSFW
-   [FlowGPT](https://flowgpt.com/) - Roleplaying Chatbots / Unlimited (Model-Ares) / Some NSFW
-   [Perchance](https://perchance.org/ai-character-chat) - Roleplaying + Images / No Sign-Up / Unlimited / Some NSFW / Resources / Lemmy
-   [PygmalionAI](https://pygmalion.chat/) - Roleplaying Chatbots / Unlimited (Free Models) / Sign-Up / Guide / Resources, 2
-   [SillyTavern](https://docs.sillytavern.app/) - Desktop App / Presets / Themes
-   [4thWall AI](https://beta.4wall.ai/) - Roleplaying Chatbots / No Sign-Up
-   [AI Dungeon](https://aidungeon.com/) - Roleplaying + Story Chatbot / No Sign-Up
-   [Crossing the Uncanny Valley](https://www.sesame.com/research/crossing_the_uncanny_valley_of_voice#demo) - Realistic AI Voice Chatlooks
-   [HammerAI](https://www.hammerai.com/characters) - Chatbot Builders + Roleplaying + Images / Unlimited / Some NSFW
-   [KoboldAI](https://koboldai.com/) - GUI for Roleplaying Chatbots / No Sign-Up
-   [KoboldAI Web](https://lite.koboldai.net/) - Roleplaying + Images / No Sign-Up / Unlimited / Some NSFW
-   [Miku](https://docs.miku.gg/)
-   [Sukino-Findings](https://rentry.org/Sukino-Findings) - AI Roleplay Resources
-   [TavernAI](https://tavernai.net/) - Roleplaying / Story Chatbot / Colab
-   [⁠your ai slop bores me](https://youraislopbores.me/) - Interactive Human AI Roleplaying

### AI Writing Tools

-   [TextFX](https://textfx.withgoogle.com/) - AI Creative Writing Tools / No Sign-Up
-   [Toolbaz](https://toolbaz.com/) - Online AI Text Tools
-   [Quarkle](https://quarkle.ai/) - AI Writing Assistant

### Video Generation

-   [⁠GeminiGen AI](https://geminigen.ai/app/video-gen) - Veo 3.1 / Sora 2 / Grok / Meta (Image to Video) / Login required
-   [Qwen](https://chat.qwen.ai/) - Qwen3.6-Plus / Qwen3.5-Plus / Sign-Up
-   [AIFreeVideo](https://aifreevideo.com/) - Unlimited / MiniMax Video-01 / Sign-Up
-   [Arena](https://arena.ai/?mode=direct&chat-modality=search) - AI Search Engine / Sign-Up / Reset Limits w/ Temp Mail / OpenAI Bridge
-   [Bing Create](https://www.bing.com/images/create/ai-video-generator) - Sora 1 (10 Fast, Unlimited Slow- Per Day) / Image to Video / Sign-Up
-   [Design Arena](https://www.designarena.ai/) - Multiple Models / Video Editing / Random Models / Reset Limits / w Temp Mail
-   [Dreamina](https://dreamina.capcut.com/ai-tool/home?type=video&workspace=0)
-   [Eggnog](https://www.eggnog.ai/) - AI Character Video Remixer + Editor
-   [FramePack](https://github.com/colinurbs/FramePack-Studio) - Desktop App / Local Models / Requires NVIDIA GPU
-   [Genmo](https://www.genmo.ai/) - 30 Monthly
-   [Google Flow](https://labs.google/fx/tools/flow) - Veo 3.1 / Veo 3.1 Lite / 50 Credits Daily / Google Sign-Up
-   [⁠HunyuanVideo](https://aistudio.tencent.com/modelSquare/home/play?modelId=303&from=/visual) - Unlimited / Sign-Up
-   [⁠Klipy](https://klipy.com/create/gif-maker/) - Veo 3 / GIF Only / 4s-5s / 4 Daily / Sign-Up
-   [Meta AI](https://www.meta.ai/) - Muse Spark / Meta Sign-Up / Unlimited
-   [ModelScope Video](https://modelscope.ai/civision/videoGeneration) - Wan 2.2 14B / 3 Daily
-   [⁠Pinokio](https://pinokio.co/) - Plugin-Based / Self-Hosted / NVIDIA Required
-   [PixVerse](https://pixverse.ai/) - 3 Daily
-   [Pollinations Chat](https://pollinations.ai/play) - Grok 4.20 Non-Reasoning / GLM-5.1 / GPT-5.4 / Multiple Models / Limits and Models
-   [VBench](https://huggingface.co/spaces/Vchitect/VBench_Leaderboard) - Video Generation Model Leaderboard
-   [Wan AI](https://create.wan.video/explore/video/generate?model=wan2.7) - Wan 2.7 / Image to Video / 10 Credits Daily (Fast Credit Based, Slow Unlimited)

### Image Generation

-   [⁠GeminiGen AI](https://geminigen.ai/app/imagen) - Nano Banana Pro / Nano Banana 2 / Grok Imagine / Sign-up
-   [⁠Hunyuan Image Generation](https://aistudio.tencent.com/chat/HunyuanDefault?modelId=Hunyuan-Image-3.0-Instruct) - Generating / Editing / 3D / Sign-Up / Unlimited
-   [Arena Text to Image](https://arena.ai/leaderboard/text-to-image) - Image Gen Benchmarks / Leaderboards
-   [Bing Create](https://www.bing.com/images/create) - Unlimited / GPT Image 1 / Editing / Generating / Sign-Up
-   [⁠ChatGPT Image](https://chatgpt.com/images/)
-   [⁠Coze](https://space.coze.cn/) - Seadream 4.0 / 50 Daily / Sign-Up with Phone / US Select CA
-   [Diffusers Image Outpaint](https://huggingface.co/spaces/fffiloni/diffusers-image-outpaint) - AI Image Extender
-   [Dreamina](https://dreamina.capcut.com/ai-tool/home?type=video&workspace=0)
-   [Genie](https://lumalabs.ai/genie) - 3D Image Generators
-   [Google AI Studio](https://aistudio.google.com/generate-image) - Nano Banana / Google Login
-   [Google Flow](https://labs.google/fx/tools/flow) - Veo 3.1 / Veo 3.1 Lite / 50 Credits Daily / Google Sign-Up
-   [Illusion Diffusion](https://huggingface.co/spaces/AP123/IllusionDiffusion) - Illusion Artwork Generator
-   [imgsys](https://imgsys.org/) - Compare Generators / Unlimited / No Direct Mode
-   [Interactive Scenes](https://lumalabs.ai/interactive-scenes) - Generate Interactive Scenes
-   [Mage](https://www.mage.space/) - Flux Schnell
-   [⁠MAI Playground](https://playground.microsoft.ai/chat) - MAI-Image-2 / 10 Daily / Sign-Up
-   [ModelScope Images](https://modelscope.ai/civision/imageGeneration) - Qwen Image Edit / Z-Image Turbo / 100 Credits Daily / Model Cost Varies
-   [OpenSourceGen](https://opensourcegen.com/) - Hunyuan Image 3.0 / Z-Image Turbo / 1500 Credits Per Day / Sign-Up
-   [Perchance](https://perchance.org/ai-photo-generator) - Chroma-Based / Unlimited / Resources / Lemmy
-   [Recraft](https://www.recraft.ai/) - 30 Daily / Sign-Up
-   [Reve Image](https://app.reve.com/) - 20 Daily / Editing / Sign-Up
-   [TheresAnAIForThat](https://theresanaiforthat.com/@taaft/image-to-image-generator/) - Editing / Unlimited / Flux Kontext Dev
-   [Wan AI Image](https://create.wan.video/explore/image/generate?model=wan2.7-pro) - Wan 2.7 Pro / Wan 2.6 / 10 Credits Daily (Fast Credit Based, Slow Unlimited) / Sign-Up
-   [Z-GEN TURBO](https://z-gen-turbo.vercel.app/) - Z-Image Turbo
-   [⁠ZonerAI](https://zonerai.com/en/image-editor) - Editing (sign up) + Generating (no sign up) / Unlimited

#### Local Frontends

-   [Automatic1111](https://github.com/AUTOMATIC1111/stable-diffusion-webui)
-   [ComfyUI](https://www.comfy.org/)
-   [DiffusionBee](https://diffusionbee.com/) - Stable Diffusion for Mac
-   [Fooocus](https://github.com/lllyasviel/Fooocus)
-   [Invoke](https://invoke-ai.github.io/InvokeAI/)
-   [Mochi Diffusion](https://github.com/MochiDiffusion/MochiDiffusion) - Stable Diffusion for Mac
-   [Stability Matrix](https://lykos.ai/)
-   [biniou](https://github.com/Woolverine94/biniou)
-   [ComfyUI-Zluda](https://github.com/patientx/ComfyUI-Zluda)
-   [⁠Diffuse](https://huggingface.co/TensorStack/Diffuse) - Stable Diffusion for AMD Systems
-   [Dione](https://github.com/dioneapp/dioneapp)
-   [Easy Diffusion](https://easydiffusion.github.io/)
-   [Makeayo](https://makeayo.com/)
-   [⁠Pinokio](https://pinokio.co/) - Plugin-Based / Self-Hosted / NVIDIA Required
-   [Radiata](https://ddpn08.github.io/Radiata/en/)
-   [SD WebUI Forge](https://github.com/lllyasviel/stable-diffusion-webui-forge)
-   [SDNext](https://github.com/vladmandic/sdnext)
-   [StableStudio](https://github.com/Stability-AI/StableStudio)
-   [Sygil WebUI](https://sygil-dev.github.io/sygil-webui/)

#### Guides / Tools

-   [A Traveler's Guide to the Latent Space](https://sweet-hall-e72.notion.site/A-Traveler-s-Guide-to-the-Latent-Space-85efba7e5e6a40e5bd3cae980f30235f) - AI Art Guide
-   [AI Horde](https://stablehorde.net/) - Stable Diffusion GPU Hoarde / Interface, 2, 3
-   [Civitai](https://civitai.com/) - SD Model Index / Archive
-   [Generative AI for Beginners](https://microsoft.github.io/generative-ai-for-beginners/) - Generative AI Guides
-   [⁠ImagePromptGuru](https://imagepromptguru.net/) - Determine Image Prompts
-   [IOPaint](https://www.iopaint.com/) - Image Fill / Item Removal / Colab
-   [Paper2GUI](https://github.com/Baiyuetribe/paper2gui/blob/main/README_en.md) - AI Image Resources
-   [⁠Prompy](https://www.prompy.me/) - Prompt Indexes
-   [SD Dynamic Prompts](https://github.com/adieyal/sd-dynamic-prompts) - Extension for Automatic1111

### Audio Generation

-   [Suno](https://suno.com/) - 10 Daily / Guide
-   [⁠ACE-Step 1.5](https://huggingface.co/spaces/ACE-Step/Ace-Step-v1.5)
-   [AI Jukebox](https://huggingface.co/spaces/enzostvs/ai-jukebox)
-   [AIVA](https://aiva.ai/)
-   [Boomy](https://boomy.com/)
-   [Eapy](https://home.eapy.io/) - MIDI Sample Generator
-   [Google Flow Music](https://www.flowmusic.app/)
-   [MMAudio](https://hkchengrex.com/MMAudio/) - Generate Audio for Silent Videos / Demo / Colab
-   [Mubert](https://mubert.com/)
-   [MusicFX](https://labs.google/fx/tools/music-fx)
-   [MusicGen](https://github.com/facebookresearch/audiocraft/blob/main/docs/MUSICGEN.md)
-   [⁠MusicGPT](https://musicgpt.com/)
-   [Pack Generator](https://output.com/products/pack-generator) - Sample Pack Generator
-   [Sonauto](https://sonauto.ai/)
-   [SOUNDRAW](https://soundraw.io/)
-   [Stable Audio](https://www.stableaudio.com/)
-   [Waveformer](https://github.com/fofr/waveformer)
-   [WolframTones](https://tones.wolfram.com/)

#### Text to Speech

-   [TTS Online](https://www.text-to-speech.online/)
-   [AIVocal](https://aivocal.io/)
-   [AnyVoiceLab](https://anyvoicelab.com/long-form-text-to-speech-converter/)
-   [Audiblez](https://github.com/santinic/audiblez) - Convert Books / Documents to Audiobooks
-   [Audio-WebUI](https://github.com/gitmylo/audio-webui)
-   [⁠Audio.Z.AI](https://audio.z.ai/)
-   [AudioArena](https://audioarena.ai/)
-   [Bark](https://github.com/suno-ai/bark)
-   [⁠Cartesia](https://play.cartesia.ai/)
-   [⁠Chatterbox](https://huggingface.co/spaces/ResembleAI/Chatterbox)
-   [ElevenLabs](https://elevenlabs.io/)
-   [EmotiVoice](https://github.com/netease-youdao/EmotiVoice)
-   [FakeYou](https://fakeyou.com/)
-   [Fish Audio](https://fish.audio/)
-   [FreeTTS](https://freetts.com/)
-   [Google Illuminate](https://illuminate.google.com/) - Generate AI Conversations
-   [Google Speech Gen](https://aistudio.google.com/generate-speech)
-   [⁠GPT-SoVITS](https://github.com/RVC-Boss/GPT-SoVITS)
-   [Hume](https://www.hume.ai/)
-   [⁠Indic LLM Arena](https://arena.ai4bharat.org/#/chat) - Multiple Models / Unlimited
-   [⁠KittenTTS](https://github.com/KittenML/KittenTTS)
-   [Kokoro TTS](https://huggingface.co/spaces/hexgrad/Kokoro-TTS)
-   [⁠Kokoro-82M](https://huggingface.co/hexgrad/Kokoro-82M)
-   [Kyutai TTS](https://kyutai.org/next/tts)
-   [LazyPy](https://lazypy.ro/tts/)
-   [LOVO](https://lovo.ai/)
-   [Moe TTS](https://huggingface.co/spaces/skytnt/moe-tts)
-   [NaturalReaders](https://www.naturalreaders.com/online/)
-   [Ondoku](https://ondoku3.com/en/)
-   [Pollinations Chat](https://pollinations.ai/play) - Grok 4.20 Non-Reasoning / GLM-5.1 / GPT-5.4 / Multiple Models / Limits and Models
-   [SoundofText](https://soundoftext.com/)
-   [⁠Speechma](https://speechma.com/)
-   [Tortoise TTS](https://github.com/neonbjb/tortoise-tts)
-   [TTS-WebUI](https://ttswebui.com/)
-   [TTSOpenAI](https://ttsopenai.com/)
-   [VanillaVoice](https://www.vanillavoice.com/)
-   [VoiceCraft](https://github.com/jasonppy/VoiceCraft)

#### Voice Change / Clone

-   [Applio](https://applio.org/) - Voice Cloning / No Sign-Up
-   [RVC V2](https://github.com/RVC-Project/Retrieval-based-Voice-Conversion-WebUI/blob/main/docs/en/README.en.md) - RVC V2 Voice Cloning (Locally)
-   [Voice Changer](https://github.com/w-okada/voice-changer/blob/master/docs_i18n/README_en.md) - Real-Time Voice Changer (W-Okada) / Guide / Colab
-   [AllVoiceLab](https://www.allvoicelab.com/) - Voice Cloning
-   [AnyVoiceLab](https://anyvoicelab.com/voice-cloning/) - Voice Cloning / No Sign-Up
-   [Voice Models](https://voice-models.com/) - AI Voice Models and Guides
-   [Zyphra](https://playground.zyphra.com/audio) - Voice Cloning

#### Voice Removal / Separation

-   [MVSEP](https://mvsep.com/) - 50 Daily
-   [Audacity Effects](https://www.audacityteam.org/download/openvino/)
-   [Ezstems](https://ezstems.com/)
-   [Mazmazika](https://www.mazmazika.com/)
-   [MDX23](https://github.com/jarredou/MVSEP-MDX23-Colab_v2)
-   [MultiSong Leaderboard](https://mvsep.com/quality_checker/multisong_leaderboard) - Music & Voice Separation AI Leaderboards
-   [Music-Source-Separation](https://github.com/jarredou/Music-Source-Separation-Training-Colab-Inference)
-   [Remove Vocals](https://www.remove-vocals.com/)
-   [Splitter](https://www.bandlab.com/splitter)
-   [Ultimate Vocal Remover](https://colab.research.google.com/github/NaJeongMo/Colaboratory-Notebook-for-Ultimate-Vocal-Remover/blob/main/Vocal%20Remover%205_arch.ipynb)
-   [Vocali.se](https://vocali.se/en)
-   [VocalRemover](https://vocalremover.org/)

### AI Tools

-   [⁠Agent Reach](https://github.com/Panniantong/Agent-Reach) - Connect AI Agents to Popular Internet Platforms
-   [ChatGPT DeMod](https://github.com/4as/ChatGPT-DeMod) - Block ChatGPT Moderation Checks
-   [ChatGPT Exporter](https://greasyfork.org/en/scripts/456055) - Export Chats
-   [Every ChatGPT GUI](https://github.com/billmei/every-chatgpt-gui) - ChatGPT GUI Index
-   [GPThemes](https://github.com/itsmartashub/GPThemes) - ChatGPT Themes
-   [⁠LightSession](https://github.com/11me/light-session) - Fix ChatGPT Lag
-   [LLM](https://llm.datasette.io/) - LLM CLI
-   [LLM Model VRAM Calculator](https://huggingface.co/spaces/NyxKrage/LLM-Model-VRAM-Calculator) - LLM Requirement Calculators
-   [⁠Page Assist](https://github.com/n4ze3m/page-assist) - Extensions
-   [⁠Privatiser](https://privatiser.net/) - Anonymize Sensitive Data for AI
-   [screenpipe](https://screenpi.pe/) - AI Screen Recorder / No Sign-Up
-   [sindresorhus's Awesome ChatGPT](https://github.com/sindresorhus/awesome-chatgpt) - AI Resources
-   [tldraw computer](https://computer.tldraw.com/) - Create Component Workflows to Generate or Transform Data

#### AI Prompts

-   [BlackFriday GPTs Prompts](https://github.com/friuns2/BlackFriday-GPTs-Prompts) - Prompt Directory
-   [Leaked Prompts](https://github.com/linexjlin/GPTs) - Prompt Directory
-   [Prompt Engineering Guide](https://www.promptingguide.ai/) - Prompting Guides
-   [ChatGPT System Prompt](https://github.com/LouisShark/chatgpt_system_prompt) - Prompt Directory
-   [Gobble Bot](https://gobble.bot/) - Generate Text Files for Chatbots
-   [⁠Heretic](https://github.com/p-e-w/heretic) - AI Jailbreak / Anti-Censorship Tool
-   [InjectPrompt Companion](https://companion.injectprompt.com/) - AI Jailbreak Prompt Assistant
-   [Jailbreak Listings](https://rentry.org/jb-listing) - Prompt Directory / Jailbreaks
-   [L1B3RT4S](https://github.com/elder-plinius/L1B3RT4S) - Jailbreak Prompts
-   [promptfoo](https://www.promptfoo.dev/) - Prompt Playgrounds
-   [Tensor Trust](https://tensortrust.ai/) - Prompting Skill Games
-   [The Big Prompt Library](https://github.com/0xeb/TheBigPromptLibrary) - Prompt Directory

### AI Indexes

-   [FutureTools](https://www.futuretools.io/?pricing-model=free) - AI Directory
-   [Google Labs](https://labs.google/) - Google AI Experiments
-   [⁠LLM Explorer](https://llm-explorer.com/) - LLM Databases / Indexes
-   [ArtificialStudio](https://app.artificialstudio.ai/tools) - Multi-Tool Browser AIs
-   [GPT Demo](https://www.gptdemo.net/gpt/search?lg=en&cate=&keywords=&tags=free,&sort=popular) - AI Directory
-   [It's Better With AI](https://itsbetterwithai.com/) - AI Directory
-   [LLM Resources Hub](https://llmresourceshub.vercel.app/) - AI Directory
-   [Models.dev](https://models.dev/) - AI Model Database / Pricing
-   [YP for AI](https://www.ypforai.com/) - AI Directory

### AI Benchmarks

-   [Arena](https://arena.ai/leaderboard) - Chatbot Leaderboards / Benchmarks
-   [Artificial Analysis](https://artificialanalysis.ai/) - Chatbot, Image, and Video Benchmarks
-   [⁠Kaggle Benchmarks](https://www.kaggle.com/benchmarks) - Chatbot Leaderboards / Benchmarks
-   [Context Arena](https://contextarena.ai/) - Chatbot Leaderboards / Benchmarks
-   [⁠Epoch AI](https://epoch.ai/benchmarks/eci) - Combines Multiple AI Benchmarks
-   [LLM Stats](https://llm-stats.com/) - Chatbot Leaderboard
-   [⁠LM Council](https://lmcouncil.ai/benchmarks) - AI Benchmark Index / Comparisons
-   [OpenLM Arena](https://openlm.ai/chatbot-arena/) - Chatbot Leaderboard
-   [OpenRouter](https://openrouter.ai/rankings) - Chatbot Popularity Rankings
-   [⁠RankedAGI](https://rankedagi.com/) - Chatbot Leaderboards / Benchmarks
-   [SEAL LLM Leaderboards](https://scale.com/leaderboard) - Chatbot Leaderboards
-   [Wolfram LLM Benchmarking Project](https://www.wolfram.com/llm-benchmarking-project/) - Chatbot Leaderboards / Benchmarks

#### Specialized Benchmarks

-   [Open VLM Leaderboard](https://huggingface.co/spaces/opencompass/open_vlm_leaderboard) - VLM Benchmark Leaderboard Aggregator
-   [AI Elo](https://aielo.co/) - AI Game Competitions / Benchmarks
-   [⁠AI Roundtable](https://opper.ai/ai-roundtable) - Run Multi-Model Debates on Any Question
-   [ChessArena](https://www.chessarena.ai/) - AI Chess Benchmarks
-   [⁠DesignArena](https://www.designarena.ai/leaderboard/) - AI Design Benchmarks
-   [⁠EQ-Bench](https://eqbench.com/) - AI Emotional Intelligence Benchmarks
-   [⁠IUMB](https://pellaml.github.io/iumb/) - AI Mathematics Benchmarks / Leaderboard
-   [⁠LLMs Bullshit Benchmark](https://petergpt.github.io/bullshit-benchmark/viewer/index.html) - Broken Premise Detection Benchmarks
-   [MathArena](https://matharena.ai/) - AI Mathematics Benchmarks / Competitions
-   [⁠Simple Bench](https://simple-bench.com/) - AI Human Reasoning Benchmarks
-   [⁠SpeechMap.AI](https://speechmap.ai/) - AI Free Speech Benchmarks
-   [⁠Vals AI](https://www.vals.ai/) - AI Finance / Tax / Law Benchmarks
-   [⁠VoxelBench](https://voxelbench.ai/) - Minecraft AI Generation Benchmark
-   [⁠VPCT](https://cbrower.dev/vpct) - AI Visual Physics Benchmarks

#### Coding Benchmarks

-   [SWEBench](https://www.swebench.com/) - Software Engineering AI Leaderboard / Benchmarks
-   [⁠AIBenchmarks](https://aibenchmarks.net/) - Coding AI Leaderboard / Benchmarks
-   [Aider LLM Leaderboards](https://aider.chat/docs/leaderboards/) - Coding AI Leaderboard / Benchmarks
-   [⁠Vals AI](https://www.vals.ai/) - AI Finance / Tax / Law Benchmarks
-   [WebDev Arena](https://arena.ai/leaderboard/webdev) - Coding AI Leaderboard / Benchmarks

### Machine Learning

-   [Deep playground](https://playground.tensorflow.org/) - Neural Networks Playground
-   [Hugging Face](https://huggingface.co/) - Machine Learning Datasets / Papers
-   [100-Days-Of-ML-Code](https://github.com/Avik-Jain/100-Days-Of-ML-Code) - Machine / Deep Learning Courses
-   [AI-For-Beginners](https://github.com/microsoft/AI-For-Beginners) - Machine Learning Guides
-   [⁠aman.ai](https://aman.ai/) - Artificial Intelligence / Deep Learning Stanford Notes
-   [Approaching (Almost) Any Machine Learning Problem](https://github.com/abhishekkrthakur/approachingalmost/blob/master/AAAMLP.pdf) - Machine Learning Problem-Solving Book
-   [Awesome Generative AI Guide](https://github.com/aishwaryanr/awesome-generative-ai-guide) - LLM Research Resources
-   [Awesome Machine Learning](https://github.com/josephmisiti/awesome-machine-learning) - Machine Learning Framework Index
-   [Awesome ML](https://github.com/underlines/awesome-ml) - Machine Learning / LLM Resources
-   [ChatGPT-Next-Web](https://github.com/ChatGPTNextWeb/ChatGPT-Next-Web) - Cross-Platform ChatGPT / Gemini UI
-   [Deep ML](https://www.deep-ml.com/) - Solve Machine Learning Problems
-   [DeepSpeed](https://www.deepspeed.ai/) - Deep Learning Optimization Library
-   [LLM Visualization](https://bbycroft.net/llm) - Learn Machine Learning
-   [Markdown Web Browser](https://github.com/Dicklesworthstone/markdown_web_browser) - Webpage > Markdown Converter
-   [minitorch](https://github.com/minitorch/minitorch) - Machine Learning Engineering Course Code
-   [ML Course Notes](https://github.com/dair-ai/ML-Course-Notes) - Machine Learning Course Notes
-   [ML YouTube Courses](https://github.com/dair-ai/ML-YouTube-Courses) - Machine Learning Courses on YouTube
-   [MMDeploy](https://mmdeploy.readthedocs.io/en/latest/) - Deep Learning Model Deployment Toolset
-   [ModelScope Finetuning](https://modelscope.ai/civision/modelTraining) - Image Gen Model Finetuning / Up to 100 Images Per LoRA
-   [Netron](https://github.com/lutzroeder/netron) - Visualizer for Neural Network, Deep Learning, and Machine Learning Models
-   [Nixified](https://nixified.ai/) - Nix Flake for AI Projects
-   [⁠Transformer Explainer](https://poloclub.github.io/transformer-explainer/) - ⁠Transformer Visualization
-   [⁠Unsloth](https://unsloth.ai/docs/new/studio) - LLM Finetuning / Text Completion / Notebooks / Guide

[⬆️ Back to README](./README.md)
